 ...
 *  
 * O professor Girafales organizou a confecção de um uniforme para 
 * as turmas da escola para comemorar o final do ano. Após algumas 
 ...
 * aquela turma. As próximas N*2 linhas contêm informações de cada um 
 * dos uniformes (serão duas linhas de informação para cada uniforme). 
 * A primeira linha irá conter o nome do estudante e a segunda linha 
 * irá conter a cor do uniforme ("branco" ou "vermelho") seguido por 
 * um espaço e pelo tamanho do uniforme "P" "M" ou "G".
 * A entrada termina quando o valor de N for igual a zero (0) e está 
 * valor não deverá ser processado.
 * 
 * Saída
 * 
 * Para cada caso de entrada deverão ser impressas as informações 
 * ordenadas pela cor em ordem ascendente, seguido pelos tamanhos 
 * em ordem descendente e por último por ordem ascendente de nome, 
